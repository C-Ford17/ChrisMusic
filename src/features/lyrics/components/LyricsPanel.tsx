...
export function LyricsPanel() {
  const { lyrics, progress, isLyricsLoading, currentSong, updateLyrics } = usePlayerStore();
                ...
                searchResults.map((res) => (
                  ...
                  >
                    <div className="flex-1 min-w-0 pr-6">
                      <p className="text-black dark:text-white font-black text-lg truncate group-hover:text-[#7C3AED] transition-colors tracking-tight">{res.trackName}</p>
                      ...
                      <div className="flex items-center gap-3 mt-3">
                        ...
                        {res.syncedLyrics && (
                          <span className="text-[10px] bg-[#7C3AED]/10 text-[#7C3AED] px-2.5 py-1 rounded-lg font-black tracking-widest flex items-center gap-1.5 border border-[#7C3AED]/20">
                            <Check size={12} strokeWidth={4} /> SINCRONIZADA
                          </span>
                        )}
                      </div>
                    </div>
                    <div className="w-12 h-12 flex items-center justify-center bg-[#7C3AED] text-white rounded-xl shadow-lg opacity-0 lg:group-hover:opacity-100 transition-all scale-75 group-hover:scale-100">
                      <Music size={20} />
                    </div>
                  </button>
                ))
        ...
        {lyrics && lyrics.length > 0 ? (
          lyrics.map((line, index) => (
            <motion.p
              ...
              initial={{ opacity: 0.1 }}
              animate={{ 
                opacity: activeLineIndex === index ? 1 : 0.15,
                scale: activeLineIndex === index ? 1.05 : 1,
              }}
              transition={{ duration: 0.5, ease: "easeOut" }}
              className={`text-2xl md:text-3xl lg:text-5xl font-black leading-tight transition-all cursor-default snap-center tracking-tighter ${
                activeLineIndex === index 
                ? 'text-[#7C3AED] dark:text-white drop-shadow-2xl' 
                : 'text-black dark:text-white/40 hover:opacity-50'
              }`}
            >
              {line.text}
            </motion.p>
          ...
          <div className="flex flex-col items-center justify-center h-full opacity-20 space-y-8 py-20">
            <div className="w-24 h-24 bg-black/5 dark:bg-white/5 rounded-full flex items-center justify-center animate-bounce">
              <Music size={48} className="text-[#7C3AED]" />
            </div>
            ...
            <button 
              ...
              className="px-10 py-4 bg-[#7C3AED] hover:bg-violet-500 rounded-2xl text-white text-xs font-black uppercase tracking-[0.2em] shadow-xl shadow-[#7C3AED]/20 transition-all border border-transparent active:scale-95"
            >
              Buscar ahora
            </button>
          </div>
