...
export default function SearchPage() {
  ...
  const performSearch = async (searchTerm: string, newOffset = 0) => {
    ...
    try {
      ...
      if (isTauri) {
        ...
        const res = await invoke<Song[]>('search_youtube_native_cmd', { query: searchTerm });
        if (Array.isArray(res)) { setResults(res); LibraryService.recordSearch(searchTerm); }
        return;
      }

      const response = await CapacitorHttp.get({
        url: `${apiUrl}/search`,
        params: { q: searchTerm, count: String(PAGE_SIZE), offset: String(newOffset) },
      });
      ...
      if (response.status === 200 && Array.isArray(response.data) && response.data.length > 0) {
        ...
        if (!isLoadMore) LibraryService.recordSearch(searchTerm);
      } else if (!isLoadMore) {
        toast.error("No se encontraron resultados");
        setHasMore(false);
      } else {
        setHasMore(false);
        toast.success("No hay más resultados");
      }
    } catch (err: unknown) {
          ...
          >
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (!e.target.value) { setResults([]); setHasMore(false); }
              }}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  e.preventDefault();
          ...
          <>
            ...
            {hasMore && (
              <div className="flex justify-center mt-10">
                <button
                  onClick={() => performSearch(query, offset)}
                  disabled={loadingMore}
                  className="flex items-center gap-3 px-10 py-4 bg-black/5 dark:bg-white/5 hover:bg-[#7C3AED]/10 hover:text-[#7C3AED] border border-black/10 dark:border-white/10 rounded-full font-black text-sm uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50"
                >
                  {loadingMore && <Loader2 size={18} className="animate-spin" />}
                  {loadingMore ? 'Cargando más...' : 'Ver más resultados'}
                </button>
              </div>
            )}
          </>
